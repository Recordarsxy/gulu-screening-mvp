...
const candidate = (id: string, name: string, company: string, role: string, summary: string, sourceRound: 'company' | 'role'): Candidate => ({
  id, jobId: 'job-1', dedupeKey: `demo-key-${id}`, name, guluId: `DEMO-${id}`, detailUrl: `http://127.0.0.1/demo/${id}`,
  currentCompany: company, currentRole: role, sourceRound,
  experiences: [{ company, role, period: '2021-至今', summary }],
});
...
export const demoRoleRound: Candidate[] = [
  ...
  candidate('demo-9','候选人壬','宏达机械','行业销售','完成大型制造企业客户拓展并参与投标。','role'),
