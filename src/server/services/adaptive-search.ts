...
function enumerateCombinations(campaign:GuluSearchCampaign,seedStepId:string):GuluFilters[]{
  ...
  const base=structuredClone(seed.filters);
...
export function planAdaptiveProbe(input:AdaptiveProbeInput):AdaptiveProbeDecision{
  ...
  const next=input.resultCount>100?available.find(filters=>dimensions(filters)>dimensions(input.currentFilters)&&includesFilters(filters,input.currentFilters)):available[0];
