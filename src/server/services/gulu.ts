...
export class GuluService {
  ...
  private recordAdaptiveProbe(id:string,stepId:string,resultCount:number,unavailableFilter?:{field:string;value:string}){
    ...
    if(decision.action==='refine'){
      ...
      this.db.prepare("UPDATE gulu_task_steps SET step_json=?,status='pending',page=1,candidate_cursor=0,last_error=NULL,updated_at=CURRENT_TIMESTAMP WHERE task_id=? AND step_id=?").run(JSON.stringify(step),id,stepId);
      this.db.prepare("UPDATE gulu_tasks SET status='running',phase='search',page=1,candidate_cursor=0,last_error=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(id);
      return{task:this.getTask(id),action:'refine' as const,step,resultCount,rationale:decision.rationale};
    }
    if(decision.action==='next_step'){const nextTask=this.completeStep(id,stepId,resultCount===0);return{task:nextTask,action:'next_step' as const,step:this.getCurrentCampaignStep(id),resultCount,rationale:decision.rationale};}
    return{task:this.getTask(id),action:'read' as const,step:current,resultCount,rationale:decision.rationale};
  }
  ...
  recordSearchFit(id:string,candidateId:string,stepId:string,input:unknown):GuluConnectorTask {const fit=GuluSearchFitSchema.parse(input);const existing=this.db.prepare('SELECT score FROM gulu_search_fits WHERE task_id=? AND candidate_id=?').get(id,candidateId) as {score:number}|undefined;this.db.prepare(`INSERT INTO gulu_search_fits(task_id,candidate_id,step_id,score,evidence_json,gaps_json,dimensions_json,verification_questions_json,policy_version,model,input_tokens,output_tokens) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(task_id,candidate_id) DO UPDATE SET step_id=excluded.step_id,score=excluded.score,evidence_json=excluded.evidence_json,gaps_json=excluded.gaps_json,dimensions_json=excluded.dimensions_json,verification_questions_json=excluded.verification_questions_json,policy_version=excluded.policy_version,model=excluded.model,input_tokens=excluded.input_tokens,output_tokens=excluded.output_tokens`).run(id,candidateId,stepId,fit.score,JSON.stringify(fit.evidence),JSON.stringify(fit.gaps),JSON.stringify(fit.dimensions),JSON.stringify(fit.verificationQuestions),fit.policyVersion,fit.model,fit.inputTokens,fit.outputTokens);if(fit.score>=70&&(!existing||existing.score<70)){this.db.prepare('UPDATE gulu_tasks SET shortlisted_count=shortlisted_count+1 WHERE id=?').run(id);this.db.prepare('UPDATE gulu_task_steps SET high_fit_count=high_fit_count+1 WHERE task_id=? AND step_id=?').run(id,stepId)}return this.getTask(id);}
