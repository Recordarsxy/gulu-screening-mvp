...
export class ScreeningEngine {
  ...
  async assessCandidate(jobId:string,ruleVersion:number,candidate:Candidate,signal?:AbortSignal):Promise<{candidateId:string;decision:Decision;created:boolean}> {
    ...
    this.db.prepare(`INSERT INTO candidates
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(job_id,dedupe_key) DO UPDATE SET current_company=excluded.current_company,current_role=excluded.current_role,experiences_json=excluded.experiences_json`)
      .run(candidate.id,jobId,candidate.dedupeKey,candidate.name,candidate.guluId??null,candidate.detailUrl??null,candidate.currentCompany,candidate.currentRole,JSON.stringify(candidate.experiences),candidate.sourceRound,buildCacheKey({},candidate.experiences));
    const canonical=this.db.prepare('SELECT id FROM candidates WHERE job_id=? AND dedupe_key=?').get(jobId,candidate.dedupeKey) as {id:string};
