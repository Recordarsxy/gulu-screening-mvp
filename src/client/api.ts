...
export type JobChangeNote={id:string;jobId:string;text:string;createdAt:string;appliedRuleVersion:number|null};
export type GuluRoundStatus='pending'|'running'|'completed'|'empty'|'failed';
export type GuluPlan={jobId:string;ruleVersion:number;version:number;sourceNotes:string;status:'draft'|'confirmed';confirmedAt:string|null;createdAt:string;updatedAt:string;rollout:{dryRunCompleted:boolean;pilotCompleted:boolean};rounds:[{kind:'company';limit:number;filters:GuluFilters},{kind:'role';limit:number;filters:GuluFilters}]};
export type GuluTask={id:string;jobId:string;ruleVersion:number;planVersion:number;mode:'dry-run'|'pilot'|'formal';status:string;currentRound:'company'|'role';page:number;candidateCursor:number;readCount:number;roundReadCount:number;dedupedCount:number;analyzedCount:number;inputTokens:number;outputTokens:number;companyStatus:GuluRoundStatus;roleStatus:GuluRoundStatus;companyReadCount:number;roleReadCount:number;lastError:string|null};
...
export const api={
 ...
 listChanges:(id:string)=>json<{items:JobChangeNote[]}>(`/api/jobs/${id}/changes`),createChange:(id:string,text:string)=>json<JobChangeNote>(`/api/jobs/${id}/changes`,{method:'POST',body:JSON.stringify({text})}),integrateChanges:(id:string,changeIds:string[])=>json<JobPack>(`/api/jobs/${id}/changes/integrate`,{method:'POST',body:JSON.stringify({changeIds})}),
 ...
 importGuluPlan:(id:string,sourceNotes:string)=>json<GuluPlan>(`/api/jobs/${id}/gulu-plan/import`,{method:'POST',body:JSON.stringify({sourceNotes})}),saveGuluPlanDraft:(id:string,plan:GuluPlan)=>json<GuluPlan>(`/api/jobs/${id}/gulu-plan`,{method:'PUT',body:JSON.stringify(plan)}),
