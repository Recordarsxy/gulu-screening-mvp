...
describe('Gulu connector API',()=>{
 ...
 async function setup(onAssessment?:()=>Promise<void>){
  const provider=new DeepSeekProvider({apiKey:'test',fetcher:async(_url,init)=>{const body=JSON.parse(String(init?.body??'{}'));const payload=JSON.parse(body.messages[1].content);const assessment=String(body.messages?.[0]?.content??'').includes('招聘筛选助手');if(assessment)await onAssessment?.();const content=assessment?{label:'review',reasonCode:'NEEDS_REVIEW',evidence:['测试证据']}:payload.template?{...payload.template,summary:'AI 已分析',search_plan:[...payload.template.search_plan,'公司轮：示例科技']}:{rounds:[{kind:'company',filters:{companies:['示例科技']}},{kind:'role',filters:{roles:['产品经理']}}]};return new Response(JSON.stringify({choices:[{message:{content:JSON.stringify(content)}}]}),{status:200})}});
