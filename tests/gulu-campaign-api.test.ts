...
describe('adaptive campaign API',()=>{
  ...
  it('generates, confirms and starts a campaign without legacy rollout gates',async()=>{
    ...
    let campaignPayload:any;const ai=new DeepSeekProvider({apiKey:'test',fetcher:async(_url,init)=>{campaignPayload=JSON.parse(JSON.parse(String(init?.body)).messages[1].content);return new Response(JSON.stringify({choices:[{message:{content:JSON.stringify({summary:'多方向海外销售搜索',targetShortlist:7,steps:[
    ...
    expect(campaignPayload.original_jd).toBe('JD');
    const latest=await request('/api/jobs/job-1/gulu-campaigns/latest');expect(latest.data.id).toBe(generated.data.id);
