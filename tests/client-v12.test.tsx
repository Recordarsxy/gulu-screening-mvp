...
describe("v1.2 non-technical workflow UI", () => {
  ...
  it("keeps the v1.2 workflow as a task-scoped legacy diagnostic", async () => {
    const source = await readFile(
      new URL("../src/client/App.tsx", import.meta.url),
      "utf8",
    );
    for (const text of [
      ...
      expect(source).toContain(text);
    expect(source).toContain("旧版两轮诊断入口");
  ...
  it("no longer renders the old manual readiness gate by default", async () => {
    const source = await readFile(
      new URL("../src/client/App.tsx", import.meta.url),
      "utf8",
    );
